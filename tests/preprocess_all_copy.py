...
class FootballDataPipeline:
    ...
    def _create_h2h_features(self, df: pd.DataFrame, recent_matches_window: int = 5) -> pd.DataFrame:
        ...
        for window in self.config['rolling_windows']:
            for feature in features:
                ...
                if feature.startswith('home_'):
                    ...
                    # Option 2: If the neutral feature should be team-specific
                    # (Example for possession_difference if you want home team's perspective)
                    # home_team_rolling = df.groupby(['season', 'home_team_id'])[feature] \
                    #     .transform(lambda x: x.rolling(window, min_periods=1).mean())
                    # away_team_rolling = df.groupby(['season', 'away_team_id'])[feature] \
                    #     .transform(lambda x: x.rolling(window, min_periods=1).mean())
                    # df[f"home_perspective_{rolling_col}"] = home_team_rolling
                    # df[f"away_perspective_{rolling_col}"] = away_team_rolling
