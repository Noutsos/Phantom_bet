...
class FootballDataPipeline:
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        missing_data = ft_games[
            ['home_shots_on_goal', 'home_shots_off_goal', 
    ...
    def _create_h2h_features_2(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _create_h2h_features_3(self, df: pd.DataFrame, LEAGUES: dict) -> pd.DataFrame:
        """Enhanced H2H calculator that uses country/competition context from LEAGUES dict"""
        df = df.copy()
        df = df.sort_values('date')
        print(df.isnull().sum())
        
        # First add competition_type and country columns based on LEAGUES dictionary
        def get_competition_info(league_id):
            for country, competitions in LEAGUES.items():
                for comp_id, comp_data in competitions.items():
                    if str(league_id) == comp_id:
                        comp_type = 'cup' if 'cup' in comp_data['name'].lower() else 'league'
                        return country, comp_type
            return 'Unknown', 'league'  # default
        
        # Add country and competition_type columns
        df[['country', 'competition_type']] = df['league_id'].apply(
            lambda x: pd.Series(get_competition_info(x)))
        ...
        for col in h2h_features:
            df[col] = 0.0 if not col.endswith('_streak') else 0  # Initialize streak as numeric 0
        
        # Pre-calculate competition averages with country context
        comp_avg_goals = {}
        comp_home_win_rates = {}
        
        for country, competitions in LEAGUES.items():
            for comp_id, comp_data in competitions.items():
                comp_matches = df[(df['league_id'] == int(comp_id)) & (df['country'] == country)]
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Calculating H2H"):
            ...
            away_team = row['away_team_id']
            current_date = row['date']
            competition_id = str(row['league_id'])
            country = row['country']
            comp_type = row['competition_type']
            comp_key = f"{country}_{competition_id}"
            
            # Get historical matches with country context
            past_matches = df[
                ((df['home_team_id'] == home_team) & 
                (df['away_team_id'] == away_team)) |
                (((df['home_team_id'] == away_team) & 
            ...
            # Add flag indicating if teams are from same country
            df.at[idx, 'h2h_same_country'] = 1.0
            
            if len(past_matches) == 0:
                # Use country+competition specific averages if available
                avg_goals = comp_avg_goals.get(comp_key, 
                                            comp_avg_goals.get(f"{country}_default", 2.5))
            ...
            self._calculate_h2h_stats(df, idx, past_matches, home_team, away_team, 'h2h_')
            
            # Calculate cup-specific stats (weighted more heavily)
            if len(cup_matches) > 0:
                self._calculate_h2h_stats(df, idx, cup_matches, home_team, away_team, 'h2h_cup_')
            
            # Calculate numeric streak value
            streak_value = self._calculate_streak(past_matches, home_team)
            df.at[idx, 'h2h_streak'] = streak_value
            
            # Additional streak-based features
            if streak_value > 0:
                df.at[idx, 'h2h_win_streak'] = streak_value
                df.at[idx, 'h2h_loss_streak'] = 0
            elif streak_value < 0:
                df.at[idx, 'h2h_win_streak'] = 0
                df.at[idx, 'h2h_loss_streak'] = abs(streak_value)
            else:
                df.at[idx, 'h2h_win_streak'] = 0
                df.at[idx, 'h2h_loss_streak'] = 0
    ...
    def _create_h2h_features(self, df: pd.DataFrame, LEAGUES: dict) -> pd.DataFrame:
        """Enhanced H2H calculator using existing country column and competition context"""
        # Create working copy and filter out rows with missing shot data
        df = df.copy()
        df = df.dropna(
            subset=['home_shots_on_goal', 'home_shots_off_goal'],
            how='any'
        ).sort_values('date')
        
        # Add competition_type from LEAGUES dictionary (country already exists)
        ...
        # Add only competition_type column (country already exists)
        ...
        # Pre-calculate competition averages using existing country column
        comp_avg_goals = {}
        comp_home_win_rates = {}
        ...
        for country in df['country'].unique():
            ...
            for comp_id in country_comps:
                comp_matches = df[
                    ...
                    (df['country'] == country) &
                    (df['home_shots_on_goal'].notna()) & 
                    (df['home_shots_off_goal'].notna())
                ]
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Calculating H2H"):
            ...
            country = row['country']  # Using existing country column
            comp_type = row['competition_type']
            comp_key = f"{country}_{competition_id}"
            
            # Get historical matches using existing country column
            past_matches = df[
                (((df['home_team_id'] == home_team) & 
                (df['away_team_id'] == away_team)) |
                ((df['home_team_id'] == away_team) & 
                (df['away_team_id'] == home_team))) &
                (df['date'] < current_date) &
                (df['country'] == country) &  # Using existing country column
                (df['home_shots_on_goal'].notna()) &
                (df['home_shots_off_goal'].notna())
            ].copy()
            
            # Teams are from same country by definition since we filtered by country
            df.at[idx, 'h2h_same_country'] = 1.0
    ...
    def _calculate_h2h_stats(self, df, idx, matches, home_team, away_team, prefix):
        ...
        home_wins = len(matches[
            ((matches['home_team_id'] == home_team) & 
            (matches['home_goals'] > matches['away_goals'])) |
            ((matches['away_team_id'] == home_team) & 
            (matches['away_goals'] > matches['home_goals']))
        ])
        
        # Away wins (from perspective of current away team)
        away_wins = len(matches[
            ((matches['home_team_id'] == away_team) & 
            (matches['home_goals'] > matches['away_goals'])) |
            ((matches['away_team_id'] == away_team) & 
            (matches['away_goals'] > matches['home_goals']))
        ])
        ...
        if len(recent_matches) > 0:
            df.at[idx, f'{prefix}recent_home_wins_last5'] = len(recent_matches[
                ((recent_matches['home_team_id'] == home_team) & 
                (recent_matches['home_goals'] > recent_matches['away_goals'])) |
                ((recent_matches['away_team_id'] == home_team) & 
                (recent_matches['away_goals'] > recent_matches['home_goals']))
            ])
            
            df.at[idx, f'{prefix}recent_away_wins_last5'] = len(recent_matches[
                ((recent_matches['home_team_id'] == away_team) & 
                (recent_matches['home_goals'] > recent_matches['away_goals'])) |
                ((recent_matches['away_team_id'] == away_team) & 
                (recent_matches['away_goals'] > recent_matches['home_goals']))
            ])
    ...
    def _calculate_streak_2(self, matches, home_team):
        """Calculate current streak from perspective of home team"""
        streak_type = None
        streak_length = 0
        
        for _, match in matches.sort_values('date', ascending=False).iterrows():
            # Determine result from perspective of current home team
            if match['home_team_id'] == home_team:
                result = 'H' if match['home_goals'] > match['away_goals'] else \
                        'A' if match['home_goals'] < match['away_goals'] else 'D'
            else:
                result = 'H' if match['away_goals'] > match['home_goals'] else \
                        'A' if match['away_goals'] < match['home_goals'] else 'D'
