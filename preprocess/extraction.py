...
class FootballDataExtractor:
    def __init__(self, log_dir="logs", base_dir: str = "data"):
    ...
    def _extract_lineups(self, data: Dict) -> pd.DataFrame:
        ...
        if 'data' not in data:
            self.logger.warning("Invalid response: Missing 'response' key")
            return pd.DataFrame()
        
        if not isinstance(data['response'], list):
            self.logger.warning("Invalid response: 'response' should be a list")
            return pd.DataFrame()

        lineup_data = []

        # Safely get fixture ID
        fixture_id = None
        try:
            parameters = data.get('parameters', {}) or {}
            fixture_id = parameters.get('fixture')
