...
def create_standings(df: pd.DataFrame) -> pd.DataFrame:
    """
    Processes football fixture data with comprehensive feature engineering including standings.
...
def calculate_h2h_features(df: pd.DataFrame, recent_matches_window: int = 5) -> pd.DataFrame:
    """Enhanced H2H calculator that works across leagues"""
    ...
    for col in h2h_features:
        df[col] = 0 if not col.startswith('h2h_streak') else ""
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Calculating H2H"):
        ...
        # Get all historical matches between these teams (any league)
        past_matches = df[
        ...
        # Wins calculation (using vectorized operations)
        home_wins = ((past_matches['home_team_id'] == home_id) & past_matches['home_winner']) | \
                   ((past_matches['away_team_id'] == home_id) & past_matches['away_winner'])
        df.at[idx, 'h2h_home_wins'] = home_wins.sum()
        
        away_wins = ((past_matches['home_team_id'] == away_id) & past_matches['home_winner']) | \
                   ((past_matches['away_team_id'] == away_id) & past_matches['away_winner'])
        df.at[idx, 'h2h_away_wins'] = away_wins.sum()
        ...
        df.at[idx, 'h2h_home_goals'] = home_goals.mean()
        df.at[idx, 'h2h_away_goals'] = away_goals.mean()
        df.at[idx, 'h2h_goal_diff'] = home_goals.mean() - away_goals.mean()
        ...
        if len(recent_matches) > 0:
            df.at[idx, f'h2h_recent_avg_goals_last{recent_matches_window}'] = \
                (recent_matches['home_goals'] + recent_matches['away_goals']).mean()
...
def create_combined_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Creates both standings and head-to-head features in the correct order.
    Standings features are calculated first since they're needed for H2H context.
    """
    # First create standings features
    df_with_standings = create_standings(df.copy())
    
    # Then create H2H features using the standings-enhanced data
    df_with_all_features = calculate_h2h_features(df_with_standings)
...
if __name__ == "__main__":
    # Load your data
    df = pd.read_csv("data/extracted/Bundesliga/2024/fixture_events.csv")
    ...
    # Create all features
    final_df = create_combined_features(df)
    
    # Save results
    final_df.to_csv("enhanced_features.csv", index=False)