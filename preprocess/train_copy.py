...
def generate_visualizations(
    ...
    try:
        ...
        # Extract the final estimator from the pipeline for learning curve analysis
        learning_curve_estimator = get_estimator_for_learning_curve(model)
        print(f"  - Using estimator: {type(learning_curve_estimator).__name__} for learning curves")
        
        # Basic learning curve
        fig_basic = plot_learning_curve(
            estimator=learning_curve_estimator,  # Use the extracted estimator
            X=X_train,
            y=y_train,
            cv=TimeSeriesSplit(n_splits=5),
            title=f"Learning Curve - {model_type} ({feature_selection_method})"
        )
        plt.savefig(f"{artifact_paths['plots']}/learning_curve_basic.png", 
                bbox_inches='tight', dpi=300)
        plt.close()
        
        # Comprehensive learning curve
        fig_comp = plot_comprehensive_learning_curve(
            estimator=learning_curve_estimator,  # Use the extracted estimator
            X=X_train,
            y=y_train,
            cv=TimeSeriesSplit(n_splits=5),
        ...
        plt.savefig(f"{artifact_paths['plots']}/learning_curve_comprehensive.png", 
                bbox_inches='tight', dpi=300)
        plt.close()
