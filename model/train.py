...
def train_and_save_model(df: pd.DataFrame, 
    ...
    """
    Optimized training function with efficient RFECV implementation
    """
    ...
    # 5. Load best parameters from previous grid search
    best_params = {
    ...
    # 6. Implement two-phase feature selection and modeling
    print("\nStarting two-phase feature selection and modeling...")
    
    # Phase 1: Preprocess data and run RFECV separately
    print("Phase 1: Running RFECV for optimal feature selection...")
    preprocessor.fit(X)
    X_processed = preprocessor.transform(X)
    
    # Create base estimator with best parameters
    ...
    # Run RFECV with reduced parameters for speed
    rfecv = RFECV(
        estimator=base_estimator,
        step=3,  # Remove 3 features at each step
        cv=TimeSeriesSplit(n_splits=3),  # Fewer splits for feature selection
        scoring='roc_auc_ovr',
        min_features_to_select=top_n_features//2,
        n_jobs=-1,
    ...
    print(f"\nOptimal number of features selected: {optimal_features}")
    
    # Phase 2: Create final pipeline with fixed feature count
    print("\nPhase 2: Creating final pipeline with optimal features...")
    ...
    # 7. Time-series cross-validation with final model
    print("\nEvaluating final model with time-series cross-validation...")
    cv_metrics = []
    feature_importances = []
    ...
    for fold, (train_idx, test_idx) in enumerate(tscv.split(X)):
        ...
        try:
            ...
            metrics = {'fold': fold + 1, 'error': str(e)}
        ...
        if fold == 0 and 'error' not in metrics:
            try:
                ...
                feature_names = get_feature_names_from_pipeline(final_pipeline, numerical_features, categorical_features)
                selected_mask = selector.get_support()
                importances = selector.estimator_.feature_importances_
                feature_importances = list(zip(
                    [name for name, selected in zip(feature_names, selected_mask) if selected],
                    [imp for imp, selected in zip(importances, selected_mask) if selected]
                ))
            except Exception as e:
    ...
    y_proba = final_pipeline.predict_proba(X)
    ...
    # Save RFECV visualization
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, len(rfecv.grid_scores_) + 1), rfecv.grid_scores_, marker='o')
    plt.xlabel("Number of features selected")
    plt.ylabel("Cross-validation score (ROC-AUC OVR)")
    plt.title("RFECV Performance")
    plt.savefig('artifacts/plots/rfecv_performance.png')
    plt.close()
