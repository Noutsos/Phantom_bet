...
class TrainPipeline:
    ...
    def setup_directories_2(self, model_type, feature_selection_method):
    ...
    def prepare_features(self, df, target_col):
        ...
        leakage_columns = {
            ...
            'total_yellow_cards', 'total_red_cards', 'total_corners', 'total_goals'
        } & set(X.columns)
    ...
    def build_pipeline_2(self, model_type, feature_selection_method, top_n_features=30):
    ...
    def train_2(self, df, target_col='outcome', model_type='random_forest',
        ...
        try:
            ...
            print_classification_report_2(model_type=model_type, feature_selection_method=feature_selection_method)
    ...
    def train(self, df, target_col='outcome', model_type='random_forest',
        ...
        try:
            ...
            # Save model and artifacts
            self.save_artifacts(X_train_full, y_train_full, X_holdout, y_holdout, target_col)
    ...
    def save_artifacts_2(self, X_train, y_train, X_test, y_test):
    ...
    def save_artifacts(self, X_train, y_train, X_test, y_test, target_col=None):
        ...
        try:
            ...
            if hasattr(estimator, 'feature_importances_'):
                ...
                importance_filename = f"{target_col}_feature_importances.csv" if target_col else "feature_importances.csv"
    ...
    def generate_visualizations(self, X_test, y_test, target_col='outcome'):
        ...
        # Generate visualizations using the actual model
        generate_basic_visualizations_3(
            pipeline=self.model,  # Pass the actual trained model pipeline
            X_test=X_test,
            y_test=y_test,
            ...
            final_metrics=self.final_metrics  # Pass the entire metrics dict if needed
        )
    ...
    def print_model_report(self, model_type, feature_selection_method, target_col=None):
        ...
        try:
            ...
            if task_type == 'classification':
                self.print_classification_report(final_metrics, cv_results, base_path)
            else:
    ...
    def print_classification_report(self, final_metrics, cv_results, base_path):
        ...
        print(f"F1 Macro:    {final_metrics.iloc[0]['holdout_f1_macro']:.3f}")
        print(f"Precision:   {final_metrics.iloc[0]['holdout_precision_macro']:.3f}")
        print(f"Recall:      {final_metrics.iloc[0]['holdout_recall_macro']:.3f}")
...
def generate_basic_visualizations_2(pipeline, X_test, y_test, artifact_paths):
    """
...
def generate_basic_visualizations_3(pipeline, X_test, y_test, artifact_paths, target_col=None, 
    ...
    try:
        model = pipeline  # Now pipeline is the actual model
        ...
        class_names = None
        if task_type == 'classification' and le is not None:  # Use le parameter, not pipeline.le
        ...
        if task_type == 'classification':
            ...
            try:
                ...
                final_estimator = model.steps[-1][1]  # Get the final estimator
            ...
            # Create inset for residual distribution
            ax_main = plt.gca()
            ax_inset = ax_main.inset_axes([0.6, 0.6, 0.35, 0.35])
            ax_inset.hist(residuals, bins=20, alpha=0.7, edgecolor='black', density=True)
            ax_inset.axvline(x=0, color='r', linestyle='--', alpha=0.8)
            ax_inset.set_title('Residual Dist.', fontsize=8)
            ax_inset.tick_params(labelsize=6)
            
            # Main residual plot
            plt.scatter(y_pred_array, residuals, alpha=0.6, s=30)
            ...
            # 4. Prediction Error Distribution with stats
            plt.subplot(2, 2, 4)
            errors = y_test_array - y_pred_array
            n_bins = min(30, len(np.unique(errors)))
            
            plt.hist(errors, bins=n_bins, alpha=0.7, edgecolor='black', density=True)
            plt.axvline(x=0, color='r', linestyle='--', alpha=0.8, label='Zero Error')
            
            # Add error statistics
            error_mean = np.mean(errors)
            error_std = np.std(errors)
            plt.axvline(x=error_mean, color='g', linestyle='-', alpha=0.8, label=f'Mean: {error_mean:.2f}')
            plt.axvline(x=error_mean + error_std, color='orange', linestyle=':', alpha=0.7, label=f'±1 STD')
            plt.axvline(x=error_mean - error_std, color='orange', linestyle=':', alpha=0.7)
...
def generate_basic_visualizations(pipeline, X_test, y_test, artifact_paths, target_col=None, 
    ...
    try:
        ...
        if task_type == 'classification':
            ...
            # 2. Feature Importance - FIXED VERSION
            plt.subplot(2, 2, 2)
            try:
                feature_importances = None
                ...
                if hasattr(model, 'named_steps'):
                    # Try to find the classifier or regressor step
                    if 'classifier' in model.named_steps:
                        ...
                        # If no specific name, get the last step
                        final_estimator = model.steps[-1][1]
                else:
                    # For simple pipelines without named_steps
                    final_estimator = model.steps[-1][1]
                
                # Check if the estimator has feature_importances_
                if hasattr(final_estimator, 'feature_importances_'):
                    try:
                        ...
                        if hasattr(model, 'feature_names_in_'):
                            ...
                            # Try to get feature names from the feature selection step
                            if hasattr(model, 'named_steps') and 'feature_selection' in model.named_steps:
                                ...
                                if hasattr(feature_selector, 'get_support'):
                                    # Get selected feature indices and map to original names
                                    selected_indices = feature_selector.get_support(indices=True)
                                    if hasattr(X_test_df, 'columns'):
                                        feature_names = X_test_df.columns[selected_indices].tolist()
                                    else:
                ...
                # If no feature_importances_, check for coef_ (for linear models)
                elif hasattr(final_estimator, 'coef_'):
                    try:
                        if hasattr(model, 'feature_names_in_'):
                            feature_names = model.feature_names_in_
                        else:
                            feature_names = [f'feature_{i}' for i in range(len(final_estimator.coef_[0]))]
                        
                        # For multi-class, take mean absolute coefficient across classes
                        if len(final_estimator.coef_.shape) > 1:
                            importance_scores = np.mean(np.abs(final_estimator.coef_), axis=0)
                        else:
                            importance_scores = np.abs(final_estimator.coef_)
                        ...
                        logger.warning(f"Error getting coefficients: {str(e)}")
                        feature_importances = None
...
def generate_probability_plots_2(model, X_test, y_test, class_names, artifact_paths):
    """
...
def generate_calibration_analysis_plot(self, y_test, y_proba, class_names, artifact_paths, target_col=None):
    """
    Generate additional calibration analysis plot with reliability diagram
    """
    try:
        filename_prefix = f"{target_col}_" if target_col else ""
        ...
        for i, class_name in enumerate(class_names):
            ...
            class_mask = (y_test == i)
            if np.sum(class_mask) > 20:  # Enough samples for meaningful calibration
                try:
                    fraction_of_positives, mean_predicted_value = calibration_curve(
                        class_mask.astype(int),
                        y_proba[:, i],
                    ...
                    plt.bar(mean_predicted_value, fraction_of_positives - mean_predicted_value, 
                           width=0.08, alpha=0.7, label='Calibration Error')
        ...
        plt.tight_layout()
        plt.savefig(f"{artifact_paths['plots']}/{filename_prefix}calibration_analysis.png", 
                   bbox_inches='tight', dpi=300, facecolor='white')
        plt.close()
        
    except Exception as e:
        self.logger.warning(f"Error generating calibration analysis: {str(e)}")


def generate_classification_summary_plots(self, model, X_test, y_test, class_names, artifact_paths, target_col=None):
    """
    Generate additional summary plots for classification models
    """
    try:
        filename_prefix = f"{target_col}_" if target_col else ""
        y_pred = model.predict(X_test)
        ...
        for i, class_name in enumerate(class_names):
            ...
            if np.sum(class_mask) > 0:
                ...
                class_accuracy.append((class_name, accuracy))
        
        # Sort by accuracy
        class_accuracy.sort(key=lambda x: x[1])
        names, accuracies = zip(*class_accuracy) if class_accuracy else ([], [])
        
        plt.barh(range(len(names)), accuracies)
        ...
        plt.tight_layout()
        plt.savefig(f"{artifact_paths['plots']}/{filename_prefix}classification_summary.png", 
                   bbox_inches='tight', dpi=300, facecolor='white')
        plt.close()
        
    except Exception as e:
        self.logger.warning(f"Error generating classification summary plots: {str(e)}")
...
def print_classification_report_2(model_type, feature_selection_method):
    """
