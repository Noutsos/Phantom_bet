...
class ProcessPipeline:
    ...
    def _create_standings_2(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Enhanced standings calculation with improved progress logging
        """
        df = df.copy()
        df['date'] = pd.to_datetime(df['date'])
        ...
        standings_features = [
            'home_points', 'away_points', 'home_goals_for', 'away_goals_for',
            'home_goals_against', 'away_goals_against', 'home_played', 'away_played',
            'home_wins', 'away_wins', 'home_draws', 'away_draws', 'home_losses', 'away_losses',
            'home_goals_diff', 'away_goals_diff', 'home_rank', 'away_rank'
        ]
        
        for col in standings_features:
            df[col] = 0
        
        # Create a temporary standings dictionary for this processing run
        current_standings = {}
        
        # PRE-INITIALIZE ALL TEAMS: Find all unique team/league/season combinations first
        unique_combinations = set()
        for _, row in df.iterrows():
            home_key = (row['home_team_id'], row['league_id'], row['season'])
            away_key = (row['away_team_id'], row['league_id'], row['season'])
            unique_combinations.add(home_key)
            unique_combinations.add(away_key)
        
        # Initialize all teams with zero stats
        for key in unique_combinations:
            current_standings[key] = {
                'points': 0, 'goals_for': 0, 'goals_against': 0,
                'wins': 0, 'draws': 0, 'losses': 0, 'matches_played': 0
            }
        
        total_matches = len(df)
        self.logger.info(f"Calculating standings for {total_matches} matches...")
        ...
        for idx, row in df.iterrows():
            ...
            # Set PRE-MATCH standings (should always exist due to pre-initialization)
            df.at[idx, 'home_points'] = current_standings[home_key]['points']
            df.at[idx, 'home_goals_for'] = current_standings[home_key]['goals_for']
            df.at[idx, 'home_goals_against'] = current_standings[home_key]['goals_against']
            df.at[idx, 'home_played'] = current_standings[home_key]['matches_played']
            df.at[idx, 'home_wins'] = current_standings[home_key]['wins']
            df.at[idx, 'home_draws'] = current_standings[home_key]['draws']
            df.at[idx, 'home_losses'] = current_standings[home_key]['losses']
            df.at[idx, 'home_goals_diff'] = current_standings[home_key]['goals_for'] - current_standings[home_key]['goals_against']
            
            df.at[idx, 'away_points'] = current_standings[away_key]['points']
            df.at[idx, 'away_goals_for'] = current_standings[away_key]['goals_for']
            df.at[idx, 'away_goals_against'] = current_standings[away_key]['goals_against']
            df.at[idx, 'away_played'] = current_standings[away_key]['matches_played']
            df.at[idx, 'away_wins'] = current_standings[away_key]['wins']
            df.at[idx, 'away_draws'] = current_standings[away_key]['draws']
            df.at[idx, 'away_losses'] = current_standings[away_key]['losses']
            df.at[idx, 'away_goals_diff'] = current_standings[away_key]['goals_for'] - current_standings[away_key]['goals_against']
            ...
            if row['status'] in ['FT', 'AET', 'PEN']:
                try:
                    ...
                    self.logger.warning(f"Error updating standings for fixture {row['fixture_id']}: {e}")
            
            # Progress logging (your existing code)
            current_match = idx + 1
            ...
            should_log = (
                progress - last_logged_percent >= 10 or
                current_time - last_log_time >= 10 or
                current_match == total_matches
            )
            
            if should_log:
                if current_match > 0:
                    eta = (elapsed / current_match) * (total_matches - current_match)
                else:
                    eta = 0
                
                self.logger.info(
                    f"Standings progress: {progress:.1f}% "
                    f"({current_match}/{total_matches}) | "
                    f"ETA: {eta:.1f}s"
                )
        ...
        # VALIDATE THE STANDINGS
        if not self._validate_standings(df):
            self.logger.error("Standings validation failed! Calculations may be incorrect.")
        else:
            self.logger.info("Standings validation passed!")
        self.logger.info(f"Standings completed in {phase1_time:.2f} seconds")
        
        # PHASE 2: Calculate team rankings
        ranking_start_time = time.time()
        last_ranking_log_time = ranking_start_time
        last_group_logged_percent = -1  # Initialize outside the loop
    ...
    def _create_h2h_features_2(self, df: pd.DataFrame, LEAGUES: dict, monitor_teams: list = None) -> pd.DataFrame:
        """Enhanced H2H calculator with proper goal calculation"""
        ...
        def calculate_match_stats(target_df, source_df, desc: str):
            ...
            for idx in range(len(result_df)):
                ...
                result_df.iloc[idx, result_df.columns.get_loc('h2h_same_country')] = 1
                
                if not past_matches.empty:
                    # Calculate stats from the perspective of the HOME team in current match
                    total_matches = len(past_matches)
    ...
    def _get_competition_type_from_name_str(self, league_name: str) -> dict:
        ...
        cup_keywords = [
            ...
            'knockout', 'playoff', 'final', 'super cup', 'community shield'
        ]
    ...
    def _update_h2h_data_2(self, new_matches: pd.DataFrame):
        """Update H2H data with new matches"""
        for _, match in new_matches.iterrows():
            if match['status'] in ['FT', 'AET', 'PEN']:  # Only completed matches
    ...
    def _update_standings_data_2(self, new_matches: pd.DataFrame):
        """Update standings data with new matches"""
        ...
        for _, match in completed_matches.iterrows():
            ...
            key = (home_id, league_id, season)
            if key not in self.standings_data:
                self.standings_data[key] = {
                    'points': 0, 'goals_for': 0, 'goals_against': 0,
                    'wins': 0, 'draws': 0, 'losses': 0, 'matches_played': 0
                }
            
            key = (away_id, league_id, season)
            if key not in self.standings_data:
                self.standings_data[key] = {
                    'points': 0, 'goals_for': 0, 'goals_against': 0,
                    'wins': 0, 'draws': 0, 'losses': 0, 'matches_played': 0
            ...
            # Update home team
            home_key = (match['home_team_id'], league_id, season)
            ...
            # Update away team
            away_key = (match['away_team_id'], league_id, season)
    ...
    def run_incremental_pipeline_2(self, force_processing: bool = False) -> pd.DataFrame:
        ...
        # Reset standings data at the start of processing
        if force_processing:
            self.standings_data = {}
        ...
        if new_processed_data.empty and not force_processing:
    ...
    def run_incremental_pipeline_3(self, force_processing: bool = False) -> pd.DataFrame:
        """Run pipeline with incremental processing, optionally force processing"""
        ...
        for country, leagues in data_structure.items():
            for league, seasons in leagues.items():
                for season in seasons:
                    season_data = self._process_single_season(country, league, season, processed_fixtures if not force_processing else set())
                    if season_data is not None and not season_data.empty:
                        new_data = pd.concat([new_data, season_data], ignore_index=True)
        
        if new_data.empty and not force_processing:
            self.logger.info("No new data processed after filtering")
            return pd.DataFrame()
        
        # Preprocess new data FIRST (this includes basic cleaning)
        processed_new_data = new_data
        
        # Update historical data stores with the cleaned data
        self._update_h2h_data(processed_new_data)
        self._update_standings_data(processed_new_data)
        
        # Now apply feature engineering (H2H and standings calculations)
        processed_new_data = self._preprocess_and_feature_engineer(processed_new_data)
        ...
        if final_path.exists():
            try:
                ...
                if 'date' in existing_data.columns:
                    existing_data['date'] = pd.to_datetime(existing_data['date'], errors='coerce')
                if 'date' in processed_new_data.columns:
                    processed_new_data['date'] = pd.to_datetime(processed_new_data['date'], errors='coerce')
        ...
        self.logger.info(f"Incremental processing complete. Total records: {len(combined_data)}")
        self.logger.info(f"New records processed: {len(processed_new_data)}")
    ...
    def simple_progress_bar(iterable, desc="Progress"):
        total = len(iterable)
        for i, item in enumerate(iterable):
            percent = (i + 1) / total * 100
            print(f"\r{desc}: [{('#' * int(percent//2)).ljust(50)}] {percent:.1f}%", end="")
            yield item
        print()  # New line at the end

    # Usage
    for i in simple_progress_bar(range(100)):
        time.sleep(0.1)
